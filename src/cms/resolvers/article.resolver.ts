...
export class ArticleResolver {
    ...
    @Query('getAllArticle')
    async getAllArticle(obj, body: { classifyAlias: string, createdAt: string, endTime: string, title: string, username: string, top: boolean, pageNumber: number, pageSize: number }) {
        const result = await this.articleService.getAllArticle(body.classifyAlias, body.createdAt, body.endTime, body.title, body.username, body.top, body.pageNumber, body.pageSize);
    ...
    async getRecycleArticle(obj, body: { classifyAlias: string, createdAt: string, endTime: string, title: string, username: string, top: boolean, pageNumber: number, pageSize: number }) {
        const result = await this.articleService.getRecycleArticle(body.classifyAlias, body.createdAt, body.endTime, body.title, body.username, body.top, body.pageNumber, body.pageSize);
        return { code: 200, message: '查询成功!', data: result.exist, total: result.total };
    }

    @Query('getCheckArticle')
    async getCheckArticle(obj, body: { classifyAlias: string, createdAt: string, endTime: string, title: string, username: string, top: boolean, pageNumber: number, pageSize: number }) {
        const result = await this.articleService.getCheckArticle(body.classifyAlias, body.createdAt, body.endTime, body.title, body.username, body.top, body.pageNumber, body.pageSize);
        return { code: 200, message: '查询成功!', data: result.exist, total: result.total };
    }

    @Query('getArticleById')
    async getArticleById(obj, body: { id: number }) {
        const data = await this.articleService.getArticleById(body.id);
    ...
    async createArticle(obj, body: { article: InputArticle }, context) {
        ...
        await this.articleService.createArticle(body.article, context.user.id);
    ...
    async updateArticle(obj, body: { article: UpdateArticle }, context) {
        ...
        await this.articleService.updateArticle(body.article, context.user.id);
